...
(function(jQuery)
{
  ...
  jQuery.fn.railsAutocomplete = function() {
    ...
    if (jQuery.fn.on !== undefined) {
      ...
      for (i = 0; i < elems.length; i++) {
        ac = $(elems[i]);
        if(ac.data('autocomplete-dropdown') && !ac.hasClass("ui-autocomplete-input")) { 
          this.railsAutoCompleter = new jQuery.railsAutocomplete(ac);
        } else {
          $(document).on('focus',ac,handler);
        }
      }
    }
    else {
  ...
  jQuery.railsAutocomplete.fn.extend({
    init: function(e) {
      if(jQuery(e).attr('data-autocomplete-minlength')) {
       e.autocomplete({ minLength: jQuery(e).attr('data-autocomplete-minlength') });
      }
      
      if(jQuery(e).attr('data-autocomplete-dropdown')) {
        this.wrapper = $( "<span>" ).addClass("custom-combobox").insertAfter( e );
        wasOpen = false;

        $( "<a>" )
          .attr( "title", jQuery(e).attr('data-autocomplete-dropdown') )
          ...
          .mousedown(function() {
            wasOpen = jQuery(e).autocomplete( "widget" ).is( ":visible" );
          })
          .click(function(elem) {
            input = $(elem.currentTarget).parents("div.autocompleter").children("input[data-autocomplete]");
            input.focus();
            if ( wasOpen ) {
              return;
            }
      ...
      jQuery(e).autocomplete({
        ...
        change: function( event, ui ) {
            ...
            jQuery(jQuery(this).attr('data-id-element')).val(ui.item ? ui.item.id : jQuery(jQuery(this).attr('data-id-element')).val());
        ...
        search: function() {
          ...
          var minLength = 0;
          if(jQuery(e).attr('data-autocomplete-minlength') !== undefined) {
            minLength = jQuery(e).attr('data-autocomplete-minlength');
          }
        ...
        open: function(event, ui) {
          $("ul.ui-autocomplete.ui-menu .ui-menu-item:even").addClass("ui-menu-item-even");
          $("ul.ui-autocomplete.ui-menu .ui-menu-item:odd").addClass("ui-menu-item-odd");
        },
